...
export function stripFences(text) {
  ...
  for (const line of lines) {
    ...
    output.push('');
    if (new RegExp(`^[ \\t]{0,3}${marker.char}{${marker.length},}\\s*$`).test(line)) marker = null;
  }
  return output.join('\n');
}
...
export function notePaths(root, config, { includeContext = false } = {}) {
  return walk(root, config.vaultDir, { ext: '.md' }).filter(
    (relPath) =>
      !isGeneratedPath(relPath, config) && (includeContext || !isContextPath(relPath, config)),
  );
}
