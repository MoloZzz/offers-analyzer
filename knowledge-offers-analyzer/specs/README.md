...
| `../../specs/016-full-evaluation-breakdown/spec.md` | One breakdown renderer over the persisted evaluation explanation, reached from a compact alert via a **Деталі** inline button and shared by `/why` and `/check` | **Phases 1–2 implemented 2026-08-05** (T001–T014): one pure builder, all `/why` and `/check` formatters refactored onto it, and the 📋 **Деталі** button delivering the full breakdown from storage with zero source requests. Presentation-only and ungated. Phase 3 (`/check` full section layout) and phase 4 (forward-compatibility proof) remain open |
