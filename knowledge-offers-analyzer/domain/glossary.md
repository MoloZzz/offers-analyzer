...
| **Total Deal Score** | The composite ranking signal: price core (`raw × confidence × penalty`, dominant) × bounded factor modifiers (liquidity, repair-risk, negotiation, seller, positives). Shown to the operator as 0–100 + per-factor reasons. Spec `003-composite-deal-score`; liquidity + repair-risk are implemented but **intentionally inactive until the survivorship correction `k` lands** ([[0010-defer-factor-activation-until-k|ADR-0010]] — activation ships with `k` in one ParameterSet change, spec 004 Phase C). | Supersedes "Opportunity score = discount × confidence". |
