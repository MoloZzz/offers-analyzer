...
The threshold history: 0.3 → 0.15 (2026-07-16, to surface candidates; [[why-no-opportunities]]) → **0.63** (2026-07-17, so nothing under ~19% below market alerts — `score = discount%/SCALE` at full confidence, so 0.63 ≈ 19%). **`SCALE`, the soft-flag penalty, and the mileage factors are no longer fixed constants** — they live in a **versioned, active `ParameterSet`** (v1 seeded from these values), so they can be tuned/rolled back at runtime. See [[0005-versioned-parameter-sets|ADR-0005]]. `minDealScore` remains per-profile config (auto-calibration is spec 002 / E3).

**Cohort width is a sample-vs-precision trade-off.** Ideally the cohort matches the listing's *mileage band* so `delta` is fair. In practice, make+model+city+exact-year+mileage collapses the RIA sample to ~1 and the confidence gate rejects everything (the "0 opportunities" bug). The cohort definitions remain mileage-banded, make+model+year±1 nationwide, then make+model, but the live ingestion path begins with reusable year±1 and then make+model ([[0013-budget-stabilization-before-lifecycle-rechecks|ADR-0013]]): a live mileage band is usually unique per incoming car and defeats the cache. City is never used (it starves the sample). The **analytic mileage correction** (M2) compensates: fair value shifts by `(expected − actual)/10 × per10kPct` % (capped ±20%), where `expected = age × annualK` (config: 15k km/yr, 2%/10k km). Details: [[why-no-opportunities]].
