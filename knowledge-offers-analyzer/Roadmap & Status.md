...
| `017-on-demand-ai-analysis` — **phases 1–4 implemented 2026-08-06/07** | Admin-only `/analyze_ai`: structured context to a language model, strict structured output, separate budget, immutable records. Phases 1–3 (T001–T025) are built: quarantined context assembly with a hash-derived delimiter, validate-or-discard output, the non-influence and module-boundary guards, an `ai_analysis` allocation under its own source key that cannot touch the AUTO.RIA pool, an Anthropic adapter behind `AnalysisProvider`, the immutable `AiAnalysis` record, the command, and the content-hash cache (phase 4, 2026-08-07: checked before admission so a hit costs nothing, single-flighted across simultaneous taps, hits rendered with their original capture time). **Open:** phase 5 (`/ai_audit`, inline button, contradiction display) | **Ships disabled** (`AI_ANALYSIS_ENABLED=false`, zero allocation). Provider credentials, approved terms, lawfulness of sending listing content, and an agreed monthly cap are operator gates ([[0019-advisory-only-ai-analysis|ADR-0019]]). The cache is now in, so cost tracks decisions rather than taps — the operator's stated condition for the feature being worth enabling |
