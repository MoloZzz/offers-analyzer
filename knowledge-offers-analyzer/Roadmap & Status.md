...
| `017-on-demand-ai-analysis` — **complete 2026-08-07** | Admin-only `/analyze_ai`: structured context to a language model, strict structured output, separate budget, immutable records. Phases 1–3 (T001–T025) are built: quarantined context assembly with a hash-derived delimiter, validate-or-discard output, the non-influence and module-boundary guards, an `ai_analysis` allocation under its own source key that cannot touch the AUTO.RIA pool, an Anthropic adapter behind `AnalysisProvider`, the immutable `AiAnalysis` record, the command, the content-hash cache (checked before admission so a hit costs nothing, single-flighted across simultaneous taps, hits rendered with their original capture time and recorded as `cached` marker rows), admin-only `/ai_audit` with a cache-hit rate, an admin-only 🤖 inline button under alerts, and a contradiction display that shows a model claim beside a conflicting curated repair-risk entry and reconciles nothing. All phases done; nothing enabled | **Ships disabled** (`AI_ANALYSIS_ENABLED=false`, zero allocation). Provider credentials, approved terms, lawfulness of sending listing content, and an agreed monthly cap are operator gates ([[0019-advisory-only-ai-analysis|ADR-0019]]). Nothing engineering-side remains: enabling it is the four T037 operator gates plus applying the additive migration and setting a non-zero allocation |
