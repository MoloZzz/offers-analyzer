...
silence. A cache hit writes a **marker row** (`status: 'cached'`, no `output` of its own) and is
rendered marked, carrying its original capture time so a month-old opinion cannot read as fresh.
The marker exists so `/ai_audit` can report a cache-hit rate: without it the cheapest invocations
would be the only invisible ones. Markers never satisfy a lookup, which filters on `available`.
...
transient provider fault into a permanent verdict.

**Contradiction display** (SPEC-017 T033): when the model's warnings disagree with the curated
repair-risk tier for a listing — the table says LOW and the model raises a high-severity warning, or
the table says HIGH and the model raises nothing above `low` — both are shown side by side and the
conflict is named. **Nothing is reconciled and nothing is written**: the curated table is edited only
by a human, and `analysis-contradiction.ts` imports the table's reader and nothing that writes. It is
computed at reply time rather than stored, so a stored analysis stops being flagged once the table
catches up. MEDIUM is not compared — a mid-vs-mid disagreement would manufacture conflicts rather
than report them. The non-influence property is asserted, not assumed: `test/unit/analysis-non-influence.spec.ts`
