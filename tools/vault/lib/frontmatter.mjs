/**
 * Small YAML-frontmatter reader for the vault's intentionally simple schema.
 *
 * Existing notes already use `title`, `type`, and `updated`, and may also have
 * project-owned metadata such as `status` or `date`.  We preserve every scalar
 * key while understanding optional engine fields (`summary`, `code`, `rev`).
 * The engine never serializes hand-written frontmatter, so it cannot rewrite
 * or discard metadata it does not own.
 */
...
/**
 * Parse a note into generic frontmatter data and a body. Unknown metadata is
 * accepted as a scalar so the current Obsidian conventions remain canonical.
 */
export function parseFrontmatter(source) {
  ...
  for (let index = 1; index < close; index++) {
    ...
    if (!match) {
      if (/^[ \t]+/.test(line)) continue; // a value line owned by a generic YAML field
